...
export default function ColorBends({
  ...
  // The renderer is created once; prop changes are pushed through the uniform update effect below.
  ...
  useEffect(() => {
    ...
    material.uniforms.uIterations.value = Math.max(1, Math.min(5, Math.round(iterations)));
    material.uniforms.uIntensity.value = intensity;
    material.uniforms.uBandWidth.value = bandWidth;

    const toVec3 = (input: string) => {
      let colorValue = input.trim();
      const variableMatch = colorValue.match(/^var\((--[^)]+)\)$/);
      if (variableMatch) {
        colorValue = getComputedStyle(document.documentElement).getPropertyValue(variableMatch[1]).trim();
      }
